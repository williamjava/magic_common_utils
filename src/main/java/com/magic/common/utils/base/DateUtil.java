...
public class DateUtil {
	 ...
	 * System.out.println(dateToString(dateResult, FMT_Y_M_D_H_M_S)); 结果： 2014-11-25
	 * 17:07:44 2014-11-30 17:07:44
	 *
	 * @param date
	 * @param n
	 * @return
	 ...
	 * @return true相等，false不相等x
	 */
	public static boolean compare_time(Date dt1, Date dt2) throws Exception{
		boolean isEqual = false;
	...
	/**
	 * 获取传入两个时间的之间所有的月份（ 参数为2015-03-05，2015-09-01 返回 2015-03，2015-04。。。。。。2015-09 ）
	 *
	 * @param minDate
	 * @param maxDate
	 * @return
	 ...
	 * @return
	 */
	public static int differentSecondsByMillisecond(Date biggerDate, Date smallerDate) {
		int seconds = (int) ((biggerDate.getTime() - smallerDate.getTime()) / (1000));
	...
	public static String getFormatDate(Date currDate) {
		return getFormatDate(currDate, DATETIME_FORMAT_YY_MM_DD_HH_MM_SS);
	}

	/**
	 * 工商银行
	 * 得到string 类型的交易时间 格式为yyyy-MM-dd
	 * @return
	 */
	public static String getTrxAccDate(){
		return getFormatDate(new Date(), DATE_FORMAT);
	}

	/**
	 * 工商银行
	 * 得到String类型的交易时间 HH-MM-SS 格式
	 * @return
	 */
	public static String getTrxAccTime() {

		return getFormatDate(new Date(),HH_MM_SS);
	}
	/**
	 * 根据格式得到格式化后的日期
	 *
	...
	public static String getFormatDate(Date currDate, String format) {
		...
		try {
			...
			return dtFormatdB.format(currDate).trim();
		} catch (Exception e) {
